...
> 2. `/etc/apt/sources.list.d/docker.sources`
...
| [VS Code Server](./vscode-server) | VS Code 的 Web 版本，几乎包含桌面版的所有功能，安装了它，文件管理和编辑体验直接提升一大截。如果你主要语言是 js/python 的话，开发体验也是非常完整的，最大的优点是代码都在服务端，你可以在任意平台通过浏览器打开编写代码，关掉浏览器之后，在其他平台继续编写，非常方便。你甚至可以写完就直接部署在服务器上，简直是一条龙服务。 |
...
| [Dockurr](./dockurr) | 没有物理机但需要运行 Windows 应用时的解决方案，可以在 Docker 容器中运行 Windows 系统，通过 RDP 远程连接使用。 |